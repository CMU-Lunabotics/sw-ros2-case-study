...
def generate_launch_description():
    ...
    pkg_ros_gz_sim_demos = get_package_share_directory('move')
    pkg_ros_gz_sim = get_package_share_directory('ros_gz_sim')

    models_path = os.path.join(pkg_ros_gz_sim_demos, 'models')
    ign_resource_path = SetEnvironmentVariable(
        'IGN_GAZEBO_RESOURCE_PATH',
        models_path + ':' + os.environ.get('IGN_GAZEBO_RESOURCE_PATH', '')
    )

    sdf_file = os.path.join(pkg_ros_gz_sim_demos, 'models', 'vehicle2.sdf')
    ...
    gz_topic = '/model/vehicle2'
    joint_state_gz_topic = '/world/vehicle2' + gz_topic + '/joint_state'
    ...
    bridge = Node(
        ...
        arguments=[
            ...
            # Link poses relative to model frame (Gazebo -> ROS2)
